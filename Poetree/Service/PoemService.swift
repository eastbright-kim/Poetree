    ...
    func fetchPoems(completion: @escaping ([Poem], String) -> Void) {
        ...
        poemRepository.fetchPoems { poemEntities, result in
            ...
            let poemModels = poemEntities.map { poemEntity -> Poem in
                ...
                return Poem(id: id, userEmail: userEmail, userNickname: userNickname, title: title, content: content, photoId: photoId, uploadAt: uploadAt, isPrivate: isPrivate, likers: likers, photoURL: photoURL, userUID: userUID, isTemporarySaved: isTemporarySaved)
            }
